...
function showPaymentDashboard(planName) {
  if (!paymentDashboard || !planPrices[planName]) return;
  ...
  if (monthlyPriceEl) monthlyPriceEl.textContent = `${plan.monthly.price}/month`;
  if (yearlyPriceEl) yearlyPriceEl.textContent = `${plan.yearly.price}/year`;
...
function setupPlanOptionListeners() {
  ...
  planOptions.forEach(option => {
    option?.addEventListener('click', function() {
      ...
      const selectedPlanEl = document.getElementById('selectedPlan');
      if (selectedPlanEl) {
        updateBillingCycle(cycle, selectedPlanEl.textContent);
      }
    });
  });
}
...
function updateBillingCycle(cycle, planName) {
  ...
  const activeOption = document.querySelector(`.plan-option[data-cycle="${cycle}"]`);
  if (activeOption) activeOption.classList.add('active');
  
  // Update summary
  const selectedBillingEl = document.getElementById('selectedBilling');
  const totalAmountEl = document.getElementById('totalAmount');
  
  if (selectedBillingEl) {
    selectedBillingEl.textContent = cycle === 'monthly' ? 'Monthly' : 'Yearly';
  }
  
  if (totalAmountEl) {
    totalAmountEl.textContent = cycle === 'monthly' ? plan.monthly.price : plan.yearly.price;
  }
}
  ...
  if (paymentForm) {
    paymentForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (paymentDashboard) paymentDashboard.style.display = 'none';
